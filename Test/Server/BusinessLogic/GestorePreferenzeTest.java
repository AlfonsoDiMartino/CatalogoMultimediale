...
import Server.Entity.ListaPreferenze;
import Server.Entity.ListaTipologie;
import Server.Entity.PreferenzaUtente;
import Server.Entity.Tipologia;
import Server.Entity.Utente;

class GestorePreferenzeTest {
	...
	@Test
	void test_GP01() {
		assertEquals(GestorePreferenze.getListaPreferenzeUtente(null).isEmpty(), true);
	}
	...
	void test_GP02() {
		...
		ArrayList<PreferenzaUtente> lista = (ArrayList<PreferenzaUtente>) GestorePreferenze.getListaPreferenzeUtente(utente);
		assertNotNull(lista);
		
		assertEquals(lista.size(), 2);
		assertEquals(lista.get(0).getAutore().getId().toString(), Integer.toString(7));
		assertEquals(lista.get(0).getTipologia().getId().toString(), Integer.toString(1));
		assertEquals(lista.get(1).getAutore().getId().toString(), Integer.toString(12));
		assertEquals(lista.get(1).getTipologia().getId().toString(), Integer.toString(3));
	}
	...
	void test_GP03() {
		Utente utente = ListaCataloghiUtenti.getUtenteById(24);
		assertNotNull(utente);
		Autore autore = ListaAutori.getAutoreById(5);
		assertNotNull(autore);
		Tipologia tipologia = ListaTipologie.getTipologiaById(1);
		assertNotNull(tipologia);
		
		assertEquals(ListaPreferenze.aggiungiPreferenza(utente, autore, tipologia), true);
		...
		assertEquals(lista.get(1).getAutore().getId().toString(), Integer.toString(12));
		assertEquals(lista.get(1).getTipologia().getId().toString(), Integer.toString(3));
		assertEquals(lista.get(2).getAutore().getId().toString(), Integer.toString(5));
		assertEquals(lista.get(2).getTipologia().getId().toString(), Integer.toString(1));
	}
	...
	void test_GP04() {
		...
		assertEquals(GestorePreferenze.aggiungiPreferenza(null, autore, tipologia), false);
		assertEquals(GestorePreferenze.aggiungiPreferenza(utente, null, tipologia), false);
		assertEquals(GestorePreferenze.aggiungiPreferenza(utente, autore, null), false);
